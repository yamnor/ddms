...
![](https://gyazo.com/b9b761d58efd6f059dc9e968ae0c384d.png)
...
![w40](https://gyazo.com/9ecf94646807d20af9c47d2a65f41e91.png)
...
![](https://gyazo.com/165cdcc26d267e62e1c5e86ad794de10.png)
...
「格子定数」と「ポテンシャルエネルギー」の関係をグラフとして表示してみます。

```python
# 格子定数とエネルギーの関係をグラフ化します
plt.figure(figsize=(8, 5))
plt.plot(a_list, energies, 'o-', label='DFT Data')
plt.xlabel('Lattice Constant ($\AA$)')
plt.ylabel('Potential Energy (eV)')
plt.title('Si Energy vs Lattice Constant')
plt.grid(True)
plt.legend()
plt.show()
```
...
![](https://gyazo.com/e85266b4817a94f98b2ad9909c7d0c17.png)

格子定数が 5.45 〜 5.50 Å の間で、エネルギーが最小値となっていることがわかります。

エネルギーが最小値となる格子定数を特定してみます。
...
以上の結果から、第一原理計算の結果、シリコンが最も安定な構造を取るときの格子定数は 5.473 Å であることがわかりました。

この結果は、実験値 5.43 Å と比較して、0.7% の過大評価となっていますが、ほぼ良い一致であると言えます。

次に、格子定数から算出した「体積」と「ポテンシャルエネルギー」の関係をグラフとして表示してみます。
...
次章では、得られた「体積とポテンシャルエネルギーの対応データ」を解析し、「体積弾性率」を導出するプロセスについて説明します。