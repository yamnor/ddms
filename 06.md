...
GPR は、データ点とデータ点の間を「確率分布（雲のようなもの）」でつなぐ手法です。データがある場所では雲が絞られて一点に定まりますが、データがない場所では雲が広がり、予測の「自信のなさ」を表現できます。
...
「AIが次に計算すべき物質を提案する（アクティブラーニング）」などの自律的な探索技術については、**[第18章：発展実習 | 自律的な材料探索へ](blog/courses/ddms/07.md)** で、実際にコードを動かしながら体験します。
